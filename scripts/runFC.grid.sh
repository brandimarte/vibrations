...
#          ${2} :  FC input file                                        #
#          ${3} :  condor submit script                                 #
#          ${4} :  execution script                                     #
#          ${5} :  I-Smeagol executable                                 #
#                                                                       #
#  Use: ./runFC.grid.sh [FC calculation directory] [FC input file] \    #
#       [condor submit script] [execution script]                  \    #
...
then
    ...
    echo -e "vibrations: Use: ./runFC.grid.sh"                          \
	"[FC calculation directory] [FC input file] \ "
    echo -e "               [condor submit script] [execution script] \ "
    echo -e "               [I-Smeagol executable]\n"
    exit -1
fi
...
CondorLabel=`grep "CASE=" ${3} | awk '{print $1}' | cut -f2 -d"="`
CondorName="CASE=${CondorLabel}"
if [ "${CondorLabel}" == "" ]
then
    echo -e "ERROR: can't find the 'CASE=' at go submit script!\n"
    exit -1
fi
...
do
    ...
    sed "s/$CondorName/CASE=${CondorLabel}-${i}/" ${3}                  \
	> ${FCdir}FC${i}/${3}

    # Submite job.
    cd ${FCdir}FC${i}
    tar -czvf input.tgz *${pseudo} *.DM *.fdf ${5}
    tar -tzvf input.tgz
    go ${3}
    rm ${5}
    cd ../
