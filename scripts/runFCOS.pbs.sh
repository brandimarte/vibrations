...
#  Then, it changes the 'fdf' file, removing the flag 'FCwriteHS',      #
#  including 'FConlyS' and changing the geometry and number of atoms.   #
#                                                                       #
#  Input:  ${1} :  FC calculation main directory                        #
#          ${2} :  FC input file                                        #
#          ${3} :  command to run parallel jobs (e.g. 'mpirun')         #
#          ${4} :  file mapping procs (e.g. '${PBS_NODEFILE}')          #
#          ${5} :  I-SMEAGOL executable with path                       #
#                                                                       #
#  Use: ./runFCOS.pbs.sh [FC calculation directory] [FC input file]     #
#       [parallel run command] [file mapping procs]                     #
...
then
    ...
    echo -e "vibrations: Use: ./runFCOS.pbs.sh"                         \
	"[FC calculation directory] [FC input file] \ "
    echo -e "               [parallel run command]"                     \
	"[file mapping procs] [I-SMEAGOL executable]\n"
    exit -1
fi

# Time is running.
echo -e ""
echo -n "vibrations: Start of run: "
date
...
then
    echo -e "\nvibrations: ERROR: the file \"${4}\" doesn't exist or"   \
	"is not accessible!\n"
    exit -1
else
    echo -e "ok!\n"
...
#    > ${FCdir}FCfdf.tmp
#    FCfdf=`ls ${FCdir}*.fdf`
#    echo -e "vibrations: FC fdf input files found:"
#    for fdf in ${FCfdf}
#    do
#	echo -e "\t\t\t\t\t${fdf}"
#        # Copies to 'FCfdf.tmp' without comented lines.
...
#    done
#else
#    echo -e "\nvibrations: ERROR: couldn't find any FC fdf input"      \
#	"file at \"${FCdir}\"!\n"
#    exit -1
#fi
