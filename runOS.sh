...
#          ${2} :  FC input file                                        #
#          ${3} :  mpi compiler (e.g. 'mpirun')                         #
#          ${4} :  file mapping procs (e.g. '${PBS_NODEFILE}')          #
#          ${5} :  SIESTA executable with path                          #
#                                                                       #
#  Use: ./runFC.sh [FC calculation directory] [FC input file]           #
#       [mpi compiler] [file mapping procs] [SIESTA executable]         #
#                                                                       #
...
then
    ...
    echo -e "vibranal: Use: ./runOS.sh [FC calculation directory]"      \
	"[FC input file] \ "
    echo -e "               [mpi compiler] [file mapping procs]"        \
	"[SIESTA executable]\n"
    exit -1
fi
...
then
    echo -e "\nvibranal: ERROR: the file \"${4}\" doesn't exist or is"  \
	"not accessible!\n"
    exit -1
elif [ ! -r ${5} ]
then
    echo -e "\nvibranal: ERROR: the file \"${5}\" doesn't exist or is"  \
	"not accessible!\n"
    exit -1
elif [ ! -x ${5} ]
then
    echo -e "\nvibranal: ERROR: you don't have permission to execute"   \
	"\"${5}\"!\n"
    exit -1
else
    echo -e "ok!\n"
...
echo -e "\nvibranal: Including the option 'FCwriteHS .false.' at"       \
    "'pbFCrun.fdf' file.\n"
...
echo -e "vibranal: Including the option 'FConlyS .true.' at"            \
    "'pbFCrun.fdf' file.\n"
...
if [ "${displ}" == "" ]
then
    echo -e "\nvibranal: ERROR: can't find the 'MD.FCdispl' value at"   \
	"FC input file!\n"
    exit -1
fi
...
if [ "${dunit}" == "" ]
then
    echo -e "\nvibranal: ERROR: can't find the 'MD.FCdispl' unit at"    \
	"FC input file!\n"
    exit -1
fi
echo -e "${displ} ${dunit}\n"
...
then
    if [ "${aunit}" != "ang" ]
    then
	echo -e "\nvibranal: ERROR: unrecognized atomic coordinates"    \
	    "format \"${aunit}\" at"
...
then
	...
	echo -e "\nvibranal: ERROR: unrecognized displacement unit"     \
	    "\"${dunit}\" at"
	echo -e "                 FC input file! It must be \"Ang\" or" \
	    " \"Bohr\"!\n"
	exit -1
...
if [ "${natoms}" == "" ]
then
    echo -e "\nvibranal: ERROR: can't find the 'NumberOfAtoms' at FC"   \
	"input file!\n"
    exit -1
fi
...
then
    ...
    echo -e "vibranal: Copying 'AtomicCoordinatesAndAtomicSpecies' to"  \
	"'pbCoord.tmp' file.\n"
    ...
    echo -e "vibranal: Removing 'AtomicCoordinatesAndAtomicSpecies'"    \
    ...
    echo -e "vibranal: Removing '%include' lines from 'pbFCrum.fdf'.\n"
...
echo -e "vibranal: Adding '%include pbCoord.fdf' at 'pbFCrun.fdf'.\n"
...
echo -e "vibranal: Including atomic coordinates with at 'pbCoord.fdf'.\n"
...
submit="${3} -n ${cores} -machinefile ${4} ${5}"
